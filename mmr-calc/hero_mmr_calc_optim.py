...
def set_player_mmr(client, data: list[tuple[int, dict[tuple[int, int], float]]]):
    client.execute(
        """
        INSERT INTO hero_mmr_history2 (account_id, match_id, player_score)
        VALUES
        """,
        [
            {
...
if os.path.exists("hero_mmr_history.json"):
    with open("hero_mmr_history.json", "rb") as f:
    ...
    with ch_client as client:
        ...
        with open("hero_mmr_history.json", "wb") as f:
