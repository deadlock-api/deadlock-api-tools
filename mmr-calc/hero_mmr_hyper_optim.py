...
def objective(trial: Trial, matches) -> float:
    ...
    for match in tqdm(matches, desc="Processing matches"):
        for team in match.teams:
            ...
            for p_key in team.players:
                p_mmr = all_player_mmrs.get(p_key, avg_team_rank_true)
                team_ranks[p_key] = p_mmr
...
if __name__ == "__main__":
    if os.path.exists("matches_hero.cache"):
        with open("matches.cache", "rb") as f:
            matches = pickle.load(f)
    else:
        matches = get_matches()
        with open("matches_hero.cache", "wb") as f:
