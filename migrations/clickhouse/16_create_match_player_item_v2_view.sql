...
ORDER BY (toStartOfDay(start_time), hero_id, average_match_badge, match_id, item_id);

CREATE MATERIALIZED VIEW default.match_player_item_v2_mv
REFRESH EVERY 30 MINUTE TO default.match_player_item_v2
AS
SELECT
    mi.start_time AS start_time,
...
FROM
    default.match_player AS mp
INNER JOIN default.match_info AS mi ON
    mp.match_id = mi.match_id
ARRAY
JOIN items
WHERE won IS NOT NULL AND (mi.match_mode = 'Unranked' OR mi.match_mode = 'Ranked')
